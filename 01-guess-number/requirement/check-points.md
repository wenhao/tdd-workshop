...
提示:应让学员充分思考承担职责的角色，不能在未经分析之前就开始编写测试，从而忽略测试带来的驱动力，甚至忘记一些基本的命名原则和面向对象设计思想。
...
那么，我们怎么知道返回的Answer对象是合法的呢? 种做法是获取Answer的属性,然后再进行验证。那么，为了测试的验证而暴露这些属性，是否适合?
...
数?换言之,构造函数参数与对象之间的关系是什么?之所以要作为构造函数参数,就是意味着在某种场景下这些参数值应该在创建该对象 时就存在。这些参数值与对象“生死与共”,它们的生命周期是保持一 致的。如果不是,就不应该作为构造函数的参数。你觉得输入应该作为 构造函数吗?如果我要验证另一条输入应该怎么办?再创建一个 InputValidator对象吗?
