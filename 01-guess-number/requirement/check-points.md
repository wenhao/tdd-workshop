...
问题: 引入InputValidator类型是否有必要? 多数人会认为这里的验证逻辑与Answer相关，根据前面􏰀到的“信息专家模式”，似乎应该将验证逻辑放到Answer中。然而，这里的需求明确地表示了，如果输入不符合要求，就不允许创建该Answer,而是抛出异常。所以，这里的 部分验证逻辑是在创建Answer之前就应该存在，当然就不应该由Answer承担了。
